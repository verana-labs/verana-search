...
and extract the **entity bindings** for the badges of [SRCH-RES-1]: `isCorporation` MUST be true if and only if the DID **is** the declared `did` of a `Corporation` entry — determined by comparing the owner Corporation record's `did` (from the [SRCH-ENR-2a] fetch) with the hit DID, NOT by the mere presence of a resolver `corporation` object (some resolver builds return it for owned DIDs too); and `ecosystemIds` (the ids of `ecosystems[]`, the Ecosystems this DID controls). `isEcosystem` is true when that list is non-empty. When the snippet carries the card data ([SRCH-ENR-4]), `isCorporation` / `isEcosystem` / `ecosystemIds` come from the snippet directly;
