...
import type { DidCard, EcsCredential, GraphError, ResolveResponse, SearchRequest, SearchResponse } from './types'
...
export function searchGraph(config: AppConfig, request: SearchRequest, signal?: AbortSignal): Promise<SearchResponse> {
  return postJson<SearchResponse>(`${config.graphBaseUrl}/v4/graph/search`, request, signal)
}
...
export function resolveDid(config: AppConfig, did: string, cacheKeySuffix = ''): Promise<ResolveResponse> {
  ...
  const promise = (async () => {
    await acquire()
    try {
      return await postJson<ResolveResponse>(`${config.resolverBaseUrl}/v4/verifiable-trust/resolve`, {
        did,
        ecsCredentials: true,
        services: true,
        ecosystems: {},
...
export function getCorporationSignals(config: AppConfig, corporationId: number): Promise<CorporationSignals> {
  const cached = corporationCache.get(corporationId)
  if (cached) return cached

  const promise = (async (): Promise<CorporationSignals> => {
    const res = await fetch(`${config.resolverBaseUrl}/v4/corporation/get/${corporationId}?gf_data=none`)
    ...
    return {
      ...
      slashedDeposit: typeof c.slashed_deposit === 'number' ? c.slashed_deposit : null,
    }
  })()
...
function findCred(creds: EcsCredential[] | undefined, ecsSchema: string): EcsCredential | undefined {
 ...
 */
export async function buildDidCard(config: AppConfig, did: string, cacheKeySuffix: string): Promise<DidCard> {
  ...
  if (!org && !persona && service?.issuerParticipantId != null) {
    try {
      const pRes = await fetch(`${config.resolverBaseUrl}/v4/participant/get/${service.issuerParticipantId}`)
      if (pRes.ok) {
  ...
  return {
    ...
    operatorCountryCode: str(o?.countryCode) ?? str(p?.controllerCountryCode),
